...
namespace CrystallGrow
{
    public class Config
    {
        ...
        public static Func<double, double, double, double, double> GetLocalPower = (p, x, y, r) =>
        {
            ...
            //if (d < r) return p;
            return DiffuseG * p / (Math.Pow(d, 2.5));
        };
        ...
        {
            return d > 10 ? Math.Pow(d, 1.22)*0.1 : 2;
        };

        public static Func<int, int> PurposeEnergy = i => (int)Math.Ceiling(Math.Pow(i, 0.5) * 8.1);
        ...
        /// </summary>
        public static Func<int, int> ExpectCrystallCount = i => (int) Math.Ceiling(Math.Pow(i, 0.5
            ));
